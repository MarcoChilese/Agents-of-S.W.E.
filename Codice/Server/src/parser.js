...
class Parser{
    constructor(data){
        // json
        this.net = data;
        this.fieldsNeeded = ['nodes', 'states', 'parents', 'probabilities'];
    }
     ...
     */
    checkMinimumFields() {
        // If there are not exactly 4 fields return false.
        if(Object.keys(this.net).length != 4)
            return false;

        // Check each field to be exactly named.
        for(let val in this.fieldsNeeded){
            if(this.net[this.fieldsNeeded[val]] === undefined)
                throw new Error(`[Error_code] - Incorrect ${this.fieldsNeeded[val]} definition`);
        }
        return true;
    }
    ...
    checkNamedNodes(data, field) {
        if(Object.keys(data).length !== this.net.nodes.length)
            throw new Error(`[Error_code] - Incorrect number of lines in ${field} definition`);
        for(const key in this.net.nodes) {
            if (data[this.net.nodes[key]] === undefined)
                throw new Error(`[Error_code] - Missing node ${this.net.nodes[key]} in ${field} \' definition `);
        }
        return true;
    }
    ...
    checkStates(){
        ...
        for(const key in this.net.nodes){
            if(this.net.states[this.net.nodes[key]].length < 2)
                throw new Error(`[Error_code] - Error in nodes ${this.net.nodes[key]} states definition, required at least 2`);
            if(this.checkDuplicates(this.net.states[this.net.nodes[key]]))
                throw new Error(`[Error_code] - Error in node's ${this.net.nodes[key]} states, trying to define multiple times same state.`);
        }
    ...
    checkParents(){
        ...
        for(let node in this.net.parents) {

            if (this.checkDuplicates(this.net.parents[node]))
                throw new Error(`[Error_code] - Trying to define multiple times same parent in ${node}`);

            for (const parent in this.net.parents[node]) {

                // Check if all nodes in parent exits in nodes.
                if (!this.net.nodes.includes(this.net.parents[node][parent]))
                    throw new Error(`[Error_code] - Parent: ${this.net.parents[node][parent]}  does not exits as node in ${node}`);

                // Check if a node is parent of itself.
                if (this.net.parents[node][parent] === node)
                    throw new Error(`[Error_code] - Try to define ${this.net.parents[node][parent]} as parent of itself`);
    ...
    countNumberOfValue(node){
        ...
        for(const key in this.net.parents[node]){
            ans *= this.net.states[node].length;
        }
        return ans;
    }
     ...
     */
    checkProbabilities(){
        // Chekc if there are the exact number of rows and each row's name exits in nodes.
        ...
        for(const probability in this.net.probabilities){
            ...
            let numberOfStates = this.countNumberOfValue(probability);
            ...
            let prob = this.net.probabilities[probability];
            let sum = 0;
