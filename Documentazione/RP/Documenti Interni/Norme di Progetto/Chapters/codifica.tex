...
ECMAScript 6 viene stardardizzato da \textit{ECMA}\glossario\ nel giugno 2015 con la sigla "ECMA-262".\\
...
metricFindQuery(query) {
  if (!query) {
	...
	\end{lstlisting}
	%conseguenti?
	\item Se si utilizza \textit{Lodash}\glossario, per coerenza, lo si preferisca alle array function native di ES6.
\end{enumerate}
...
Per la parte front-end\glossario del plug-in il gruppo \texttt{Agents of S.W.E.} ha scelto di utilizzare i linguaggi \textit{CSS}\glossario ed \textit{HTML5}\glossario.\\
...
Insieme al \textit{CSS}, per lo sviluppo della parte front-end del progetto, il gruppo ha scelto di utilizzare \textit{HTML5}. Per la stesura della Style Guide verrà utilizzata la documentazione resa disponibile da W3C.\\
...
\paragraph{AngularJS} \label{angularjs} \-\\
...
Npm è un gestore di pacchetti per \textit{JavaScript}, gestisce inoltre i pacchetti di default presenti nell'ambiente \textit{Node.js}\glossario.\\
...
\paragraph{Webpack} \label{webpack}\-\\
Webpack è un module bundler\glossario per applicazioni javascript. Nella pratica lo scopo di webpack è creare un pacchetto di assets leggibile e utilizzabile dal browser a partire da un insieme di file sorgenti strutturati su diversi file e con dipendenze complesse. \\
...
\item \textbf{Loaders}: di default webpack esegue la build solamente di file javascript e \textit{JSON}\glossario, tramite questa configurazione webpack può essere adattato ad altri tipi di file, così da permettere la conversione anche di essi. I loaders possono avere due proprietà: 
	\begin{itemize}
