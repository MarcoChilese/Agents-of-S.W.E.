...
\paragraph{AngularJS}
\label{AngularJS}
AngularJS\footnote{\url{https://www.angularjs.org/}} è un framework Javascript per lo sviluppo di applicazioni web client side. Viene utilizzato per il reperimento di dati 
Di seguito sono riportate alcune norme da seguire:
\begin{itemize}
	\item \textbf{Norma 1}: 
	verrà seguito il pattern Model Controller View(MVC) che propone la scomposizione in tre componenti:
	\begin{itemize}
		\item Model: responsabile per il mantenimento dei dati;
	...
	\end{itemize}
	\item \textbf{Norma 3}:
	il nome dei moduli viene assegnato usando la notazione lowerCamelCase(itemListCtrl);
	\item \textbf{Norma 4}:
	quando è presente un modulo b che è submodule di a, si possono concatenere usando la notazione a.b;
	\item \textbf{Norma 5}:
	il nome dei controller viene assegnato in base alla loro funzionalità e viene usato l'UpperCamelCase(ItemListCtrl);
	\item \textbf{Norma 6}:
	i controller non devono essere definiti globali;
	\item \textbf{Norma 7}:
	per mantenere il codice HTML di più semplice lettura, le direttive AngularJS vengono messe in fondo:
	\begin{lstlisting} [language=JavaScript]
	//OK
	...
	\item \textbf{Norma 9}:
	assegnare i nomi alle direttive seguendo il lowerCamelCase;
	\item \textbf{Norma 10}:
