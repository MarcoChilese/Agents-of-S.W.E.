...
\hline
TU0-32 & Viene verificato che la richiesta di \texttt{uploadnetwork} ritorni una risposta con stato \textit{404} in caso di fallimento & N.I. \\ 
\hline
...
\hline
 ...
 \hline
 TU0-48 & Viene verificato che il metodo \texttt{addToPool(net)} ritorni aggiunga nel pool di monitoraggio la rete desiderata & N.I. \\
\hline
...
\hline 
TU0-50 & Viene verificato che il metodo \texttt{addToPool(net)} ritorni false nel caso in cui la rete da monitorare e già monitorata & N.I. \\ 
\hline
...
\hline
\rowcolor{grigio}TU0-67 &  Viene verificata la creazione di una connessione al database al richiamo del metodo \texttt{initDatabaseConnection(connection)}, ritorni \textbf{True} nel caso di esito operazione positivo & N.I.\\ 
\hline
...
\hline
\rowcolor{grigio}TU0-193 & Viene verificato che il metodo \textit{addToPool} funzioni correttamente con una nuova rete, ritorni \textbf{Not Undefined} se ha esito positivo & S.\\
\hline
TU0-194 & Viene verificato che il metodo \textit{addToPool} funzioni correttamente con una rete già nel pool, ritorni \textbf{False} se ha esito positivo & S.\\
\hline
\rowcolor{grigio}TU0-195 & Viene verificato che la connessione al server influx avvenga in maniera corretta utilizzando le credenziali di acceso, ritorni \textbf{True} se ha esito positivo & S.\\
\hline
TU0-196 & Viene verificato che la connessione al server influx avvenga in maniera corretta non utilizzando le credenziali di acceso, ritorni \textbf{True} se ha esito positivo & S.\\
\hline
\rowcolor{grigio}TU0-197 & Viene verificato il corretto funzionamento del metodo \textit{getDatasources}, ritorni un valore \textbf{Not Undefined} se ha esito positivo & S.\\
\hline
TU0-198 & Viene verificato che il metodo \textit{getDatasourcesFields} ritorni un array di tipo json, contenente valori di tipo \textit{float} del campo desiderato & S.\\
\hline
\rowcolor{grigio}TU0-199 & Viene verificato che il metodo \textit{getLastValueAsync} ritorni l'ultimo valore del campo desiderato & S.\\
\hline
TU0-200 & Viene verificato che il metodo \textit{getDatasourcesFields} ritorni un array di tipo json, contenente valori di tipo \textit{float} del campo desiderato & S.\\
\hline
\rowcolor{grigio}TU0-201 & Viene verificato che il metodo \textit{getListData} ritorni i valori del campo desiderato & S.\\
\hline
TU0-202 & Viene verificato che venga costruita una connessione con il database su cui si andrà a scrivere dati & S.\\
\hline
\rowcolor{grigio}TU0-203 & Viene verificato nel \textit{Server} che la rete da salvare sia un JSON valido & S.\\
\hline
TU0-204 & Viene verificato che la probabilità di uno stato venga assegnata a uno nel momento in cui venga superata la relativa soglia & S.\\
\hline
\rowcolor{grigio}TU0-205 & Viene verificato che la probabilità di uno stato non venga assegnata a uno nel momento in cui non venga superata la relativa soglia & S.\\
\hline
TU0-206 & Viene verificato che il metodo \textit{observeData} ritorni \textbf{True} se viene superata una soglia critica & S.\\
\hline
