...
\subsubsection{Diagramma delle Classi del Server}

Classe principale del server, utilizzata per gestire le chiamate HTTP/S in entrata e formattare una risposta a quest'ultima. 
Lo scopo principale della classe è di risolvere le dipendenze tra i vari moduli per la gestione delle reti bayesiane e il flusso di dati richiesto da quest'ultima. Inoltre gestisce le configurazioni basi, come la porta d'ascolto del server e il salvataggio delle reti bayesiane sul filesystem. \\
Il modulo InfluxDB modella un flusso di dati temporale, utilizzando come base la libreria influx, fornendo i metodi base necessari alla lettura e scrittura di dati in formato temporale. \\
La classe Network adatta la libreria \textit{jsbayes} consigliata dalla proponente per gestire le reti bayesiane. 
La classe gestisce una singola rete bayesiana per ogni istanza. \\
Il modulo ProxyServer si interpone tra il client e il server, fornendo un layer\glossario di protezione per quest'ultimo. 
Viene implementato seguendo il proxy pattern di tipo protection, per garantire un livello di sicurezza aggiuntivo lato server. 
\begin{landscape}
\begin{figure}[H]
 \begin{center}
